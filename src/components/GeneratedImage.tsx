...
import { User, Download, Share2, Loader2 } from "lucide-react";
...
export const GeneratedImage = React.forwardRef<HTMLDivElement, GeneratedImageProps>(({ imageUrl, userName, email, phone, wishText, userPhotoUrl, fontSize, textAlign, fontColor, fontFamily, userPhotoAlignment, userPhotoSize, swapImageAndText, backgroundColor }, ref) => {
  ...
  useEffect(() => {
    ...
    const handleLoad = () => calculateImageSize();
  ...
  const containerStyle: React.CSSProperties = { textAlign };
  const nameStyle: React.CSSProperties = { fontSize: `${fontSize}px`, color: fontColor, fontFamily, lineHeight: 1.2 };
  const detailsStyle: React.CSSProperties = { fontSize: `${Math.round(fontSize * 0.75)}px`, color: fontColor, fontFamily, lineHeight: 1.2 };
  const detailsContainerStyle: React.CSSProperties = { width: renderedImageWidth ? `${renderedImageWidth}px` : "100%", marginInline: "auto", backgroundColor };
  const currentAvatarSize = (userPhotoSize / 100) * 40;
  const currentIconSize = (userPhotoSize / 100) * 20;
      ...
      <CardContent className="p-0">
        <div ref={imageContentRef}>
          <div className="flex flex-col items-center">
            <div className="bg-gray-200 dark:bg-gray-800 w-full relative min-h-[200px]">
              {isLoading ? (
            ...
            <div
              className={cn("p-4 flex flex-col sm:flex-row sm:items-center sm:space-x-4 border-t w-full", { "sm:flex-row-reverse sm:space-x-reverse": swapImageAndText })}
              style={detailsContainerStyle}
            >
              {userPhotoUrl && (
                <div className={cn("flex w-full sm:flex-1", { "justify-start": userPhotoAlignment === "left", "justify-center": userPhotoAlignment === "center", "justify-end": userPhotoAlignment === "right" })}>
        ...
        <div className="p-4 border-t flex items-center justify-end gap-2">
          ...
          <Button onClick={handleShareImage} disabled={isLoading}>
